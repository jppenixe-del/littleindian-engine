// 🦅 napk9_train_v10.rs — trainer da V10 FULL THREATS (record V2 / .data2).
//    Derivado do napk9_train_juntas.rs com 5 mudanças (procura "V10:" abaixo):
//      1. inputs NapkInputV10 (módulo napk9_v10) — features on-the-fly dos bitboards
//      2. affines acc/psqt: 23169 → 31745 (22528 peças + 9216 threats + 1 dummy)
//      3. registo: 268 → 112 bytes (NapkRecordV2) no cálculo das posições
//      4. buckets: NapkV2MaterialBuckets
//      5. NAPK_DATA aponta p/ .data2 (gerados com binpack_to_data --v2)
//    INSTALAÇÃO: cp napk9_v10.rs + napk9_v10_features.rs p/ o src/ do bullet (lado a lado),
//    registar `mod napk9_v10;`, e este ficheiro p/ examples/.
use bullet_lib::{
    nn::optimiser::AdamW,
    napk9_v10::{NapkInputV10, NapkV2MaterialBuckets},   // V10: módulo novo
    trainer::{
        save::SavedFormat,
...
fn main() {
    ...
    // 🦅 s29: NAPK_THREATS = none | 640 | full (default full). Os MESMOS .data2 servem os 3
    //   modos (bitboards → features no loader). A rede sai com a geometria do modo e o
    //   auto_converter/motor detetam sozinhos.
    let threats_mode: u8 = match std::env::var("NAPK_THREATS").unwrap_or_default().as_str() {
        "none" => 0, "640" => 1, _ => 2,
    };
    // 🦅 FIX s29: o nº de inputs das affines acc/psqt TEM de seguir o modo (era cravado a 31745
    //   → panic "left==right 31745 vs 23169" quando NAPK_THREATS=640). Mesma fórmula do
    //   NapkInputV10::num_inputs (1 dummy incluído).
    let n_in: usize = 1 + match threats_mode { 0 => 22528, 1 => 23168, _ => 31744 };
    println!("🦅 NAPK_THREATS: {}", match threats_mode { 0 => "NONE (só peças 22528 — máxima velocidade)",
        1 => "640 clássico (peças + threats comprimidos — rápido)", _ => "FULL 9216 (V10 — máxima qualidade)" });
    ...
    let mut trainer = ValueTrainerBuilder::default()
        .dual_perspective()
        .optimiser(AdamW)
        .inputs(NapkInputV10 { mode: threats_mode })                 // V10:
        .output_buckets(NapkV2MaterialBuckets::<8>)   // V10:
        .save_format(&save_ids_all())
        .build_custom(move |builder, (stm, ntm, buckets), targets| {
            // ── acc + psqt UMA vez (partilhados pelas 3 cabecas) ──
            let acc_layer  = builder.new_affine("acc",  n_in, l1);   // 🦅 segue NAPK_THREATS
            let psqt_layer = builder.new_affine("psqt", n_in, 8);   // 🦅 segue NAPK_THREATS
    ...
    let data_path = std::env::var("NAPK_DATA")
        .unwrap_or_else(|_| "/mnt/d/Nap2Siriux/training/napk_sf_v2.data2".to_string());   // V10: .data2!
    let paths: Vec<&str> = data_path.split(':').filter(|s| !s.is_empty()).collect();

    // 🦅 nº de posições = soma dos tamanhos / 112   // V10: NapkRecordV2 (NapkRecord). p/ calcular épocas/superbatches.
